...
<template>
    <div class="space-y-4 p-10 bg-white rounded-xl shadow-sm">
        <h1 class="text-2xl font-bold">
            {{ t('payments.show.subscriptionMessage.title') }}
        </h1>
        <p class="text-gray-600">
            {{ t('payments.show.subscriptionMessage.description') }}
        </p>
        ...
        <div class="flex flex-wrap justify-center gap-3">
            <div
                ...
                :key="subscription.id"
                class="border flex flex-col justify-between gap-6 bg-white shadow-lg rounded-lg p-6 text-center w-full sm:w-1/2 lg:w-1/3 max-w-xs"
            >
                <div class="space-y-2">
